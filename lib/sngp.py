...
class Laplace(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            precision_matrix_minibatch: torch.Tensor = k.t() @ k
            self.precision_matrix += precision_matrix_minibatch.detach()
            self.seen_data += x.shape[0]
        else:
            # TODO: this is annoying for loading the model later
            assert self.seen_data > (self.num_data - self.train_batch_size), "not seen sufficient data"

            # TODO: cache this for efficiency
            cov = torch.inverse(self.precision_matrix)
            # cov = torch.linalg.inv(self.precision_matrix)
